...
def step_one(img_name, T1_dir, T2_dir, T1_label_dir, T2_label_dir, predictor=None):
    ...
    for i, label_dir in enumerate([T1_label_dir, T2_label_dir]):
        ...
        try:
            ...
            # # 如果中间帧不为0，将首帧输入的mask作为中间帧的输入
            # if mid_frame > 0 and "out_obj_ids" in locals() and len(out_obj_ids) > 0:
            #     for idx in range(mid_frame):
            #         mid_ann_list = []
            ...
            #             # show_mask(mid_ann_list[i]["mask"], ax1)
            #             # plt.show()

            #         for item in mid_ann_list:
            #             _, _, _ = add_new_obj(
            #                 **item, predictor=predictor, inference_state=inference_state
            #             )
